...
export default function UserProfileScreen({ navigation, route }) {
  ...
  const toggleFollow = async () => {
    ...
    try {
      ...
      notify('Could not update follow', err.message);
    } finally {
      setFollowing(false);
    }
  };
    ...
    <View style={styles.header}>
      ...
      {profile.isMe ? (
        ...
        <TouchableOpacity style={[styles.primaryBtn, profile.isFollowing && styles.followingBtn]} onPress={toggleFollow} disabled={following}
          accessibilityRole="button" accessibilityLabel={profile.isFollowing ? `Unfollow ${profile.user.name}` : `Follow ${profile.user.name}`}>
          {following ? <ActivityIndicator size="small" color={profile.isFollowing ? colors.primary : colors.white} /> : (
            <>
              ...
              <Text style={[styles.primaryBtnText, profile.isFollowing && styles.followingBtnText]}>{profile.isFollowing ? 'Following' : 'Follow'}</Text>
            </>
          )}
        </TouchableOpacity>
      )}
      {profile.isMe && (
        <View style={styles.tabs}>
          {[['posts', 'My Posts'], ['saved', 'Saved']].map(([k, label]) => (
  ...
  const emptyText = tab === 'saved'
    ? 'Posts you save with the bookmark icon appear here.'
    : profile?.isMe ? 'You haven’t posted yet.' : `${firstName(profile?.user?.name)} hasn’t posted yet.`;

  return (
    <SafeAreaView style={styles.safe}>
      <StatusBar barStyle="dark-content" backgroundColor={colors.white} />
      <ScreenHeader title={profile?.isMe ? 'My Profile' : 'Profile'} onBack={() => navigation?.goBack()} />
